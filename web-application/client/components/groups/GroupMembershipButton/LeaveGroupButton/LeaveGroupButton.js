...
const LeaveGroupButton = function({ groupId, userId }) {
    ...
    if ( userId == currentUser.id && member && member.status === 'member') {
        return (
            <>
                <RequestErrorModal message="Attempt to leave group" request={request} /> 
                <Button onClick={() => setAreYouSure(true)}><ArrowLeftStartOnRectangleIcon /> <span className="nav-text">Leave</span></Button>
