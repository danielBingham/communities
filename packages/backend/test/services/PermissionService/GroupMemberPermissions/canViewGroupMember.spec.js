const Logger = require('../../../../logger')
const FeatureFlags = require('../../../../features')

const ServiceError = require('../../../../errors/ServiceError')
const PermissionService = require('../../../../services/PermissionService')
const GroupMemberPermissions = require('../../../../services/permission/GroupMemberPermissions')

const entities = require('../../../fixtures/entities')
const database = require('../../../fixtures/database')

describe("GroupMemberPermissions.canViewGroupMember()", function() {

    const core = {
        logger: new Logger(),
        config: {
            s3: {
                bucket_url: '',
                access_id: '',
                access_key: '',
                bucket: ''
            },
        },
        database: {
            query: jest.fn()
        },
        queue: null,
        postmarkClient: {
            sendEmail: jest.fn()
        },
        features: new FeatureFlags() 
    }

    beforeEach(function() {
        core.database.query.mockReset()
        // Disable logging.
        core.logger.level = -1 
    })


    it("Should allow you to view GroupMembers if you can view 'Group:content'", async function() {
        const service = new PermissionService(core)
        const permissions = new GroupMemberPermissions(core, service)

        permissions.permissionService.can = jest.fn()
        permissions.permissionService.can.mockReturnValue(false).mockReturnValueOnce(true)

        const currentUser = entities.users.dictionary['5c44ce06-1687-4709-b67e-de76c05acb6a']

        const canViewGroupMember = await permissions.canViewGroupMember(currentUser, {})

        expect(canViewGroupMember).toBe(true)
    })

    it("Should not allow you to view GroupMembers if you can't view 'Group:content'", async function() {
        const service = new PermissionService(core)
        const permissions = new GroupMemberPermissions(core, service)

        permissions.permissionService.can = jest.fn()
        permissions.permissionService.can.mockReturnValue(false).mockReturnValueOnce(false)

        const currentUser = entities.users.dictionary['5c44ce06-1687-4709-b67e-de76c05acb6a']

        const canViewGroupMember = await permissions.canViewGroupMember(currentUser, {})

        expect(canViewGroupMember).toBe(false)
    })

})
