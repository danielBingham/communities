...
describe("GroupMemberPermissions.canCreateGroupMember()", function() {
    ...
    describe("for 'open' groups", function() {
        ...
        it("Should allow non-members to create themselves", async function() {
            ...
            const canCreateGroupMember = await groupMemberPermissions.canCreateGroupMember(currentUser, 
                { group: group, groupMember: groupMember, userMember: null })

            expect(canCreateGroupMember).toBe(true)
            expect(groupMember.groupId).toBe(group.id)
            expect(groupMember.userId).toBe(currentUser.id)
        })

        it("Should allow moderators to create members", async function() {
            const permissionService = new PermissionService(core)
            const groupMemberPermissions = new GroupMemberPermissions(core, permissionService)
        ...
        it("Should not allow non-moderators to create members for other users", async function() {
            const permissionService = new PermissionService(core)
            const groupMemberPermissions = new GroupMemberPermissions(core, permissionService)
            ...
            expect(canCreateGroupMember).toBe(false)
            expect(groupMember.groupId).toBe(group.id)
    ...
    describe("for 'hidden' groups", function() {
        ...
        it("Should not allow non-members to create themselves", async function() {
            const permissionService = new PermissionService(core)
            const groupMemberPermissions = new GroupMemberPermissions(core, permissionService)
