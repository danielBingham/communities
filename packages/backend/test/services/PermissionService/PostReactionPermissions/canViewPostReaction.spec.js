...
describe("PostReactionPermissions.canViewPostReaction()", function() {
    ...
    it("Should allow anyone to view a PostReaction", async function() {
        const service = new PermissionService(core)

        const currentUser = entities.users.dictionary['5c44ce06-1687-4709-b67e-de76c05acb6a']

        const canViewPostReaction = await service.can(currentUser, 'view', 'PostReaction')

        expect(canViewPostReaction).toBe(true)
    })
})
