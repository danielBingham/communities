 ...
 * @param {String} [options.headerName='X-Auth'] Session ID header name for token auth
 * @param {String} [options.platformHeader='X-Platform'] The name of the header to check for the platform.
...
function session(options) {
  ...
  const authHeader = opts.headerName || opts.headerKey || 'X-Auth'

  const platformHeader = opts.platformHeader || 'X-Platform'
...
function setAuthHeader(res, name, val, secret, options) {
  var signed = 's:' + signature.sign(val, secret);
